...
# ─── Proje Kökü & .env Yükleme ──────────────────────────────
_PROJECT_ROOT = Path(__file__).resolve().parent.parent

# Önce ana .env'yi yükle (ENV değişkenini okumak için)
load_dotenv(_PROJECT_ROOT / ".env")

# Ortam belirleme: ENV=production (varsayılan) veya ENV=development
ENV = os.getenv("ENV", "production").lower()
IS_DEV = ENV in ("development", "dev", "test")
IS_PROD = not IS_DEV

# Ortama özel .env dosyası varsa üstüne yükle (override)
